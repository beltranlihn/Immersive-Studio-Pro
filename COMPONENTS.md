...
| vinst + servo | `<video>`+tex por clip, servo de velocidad. **[R330]** la instancia se guarda por `c.id`, pero la FUENTE de un clip puede cambiar sin cambiar de clip (el tejido barajado rota `mediaEfId`): `vi.mid` detecta el cambio, tira el decodificador viejo, desata el `<video>` y cierra la carrera de un demux en vuelo del medio anterior | app.js · `vinstEnsure`/`ploop` | ✅ | R330 |
