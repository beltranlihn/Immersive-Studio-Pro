...
- **[R214] `purgeMediaTrash()` runs on both success paths.** `state.mediaTrash` used to only ever be wiped wholesale (new/open/newRoomProject, which already discard all media anyway) — a media deleted from the panel mid-session stayed in the trash for the rest of it. A successful save now also purges any trashed media whose id is unreferenced by every live sequence's clips AND unreachable from every per-sequence undo/redo stack (`_undoBySeq`, ≤80 snapshots total — cheap to scan). See `purgeMediaTrash` (app.js, next to `undo`/`redo`).
