...
### Recorrido guiado — `tourSteps(fmt)` / `startTour(fmt)` / `tourTrasCrear(fmt)` (app.js ~L2768)
- **Purpose:** sale **cada vez** que se crea un proyecto (domo, 2D o sala), con textos propios de cada formato. Es la presentación del programa, pensada para poder mostrarla siempre.
- **[R210]** Disparador ÚNICO: `tourTrasCrear(fmt)` al final de `newProject` (`_fl?'flat':'dome'`) y de `newRoomProject` (`'room'`). Por ahí pasan **todos** los caminos de crear: launcher (`lchCreate`), menú File → New… (que antes NO lo lanzaba) y Ctrl/Cmd+N. **No** sale al abrir un `.isp` ni un reciente (`openProjectPath`→`loadProject`, otro camino), ni si `confirmDiscard()` cancela.
- **[R178]** Antes saltaba ANTES del launcher, sobre una escena de demostración (`startOnboarding`, archivada); luego pasó a `lchCreate()` y sólo la primera vez de cada formato.
- **Invariants / gotchas:** `startTour` recibe el FORMATO, no un booleano. Espera 900 ms a que el editor esté pintado — recorta agujeros sobre elementos reales y necesita sus medidas. `_tourSkipNext` lo silencia una vez (lo usa `buildDemoProject`, que quedó sin llamantes en la app pero la usan los arneses de `scratchpad/`). **[R210]** Se eliminaron las banderas de localStorage `dspOnboardV1` y `dspTour_*` con sus seis funciones: al salir siempre ya nadie las leía (y `onboardDone()` llevaba sin lectores desde R178).
- **Status:** ✅
...
## Recorrido guiado [D7]
- **Purpose:** coach-marks sobre el editor REAL (visor → línea de tiempo → inspector → export) con textos propios de cada formato. **[R210]** Sale cada vez que se crea un proyecto; se salta con Esc / «Skip tour» y se relanza desde **Ventana → Recorrido guiado**.
- **Location:** app.js · `tourTrasCrear()` / `startTour(fmt)` / `tourSteps(fmt)` + `buildDemoProject()`/`_demoAddShape`/`_demoAddText` (bloque tras `showLanding`, ~L2173). Disparos: final de `newProject` y de `newRoomProject`; ítem de menú en `openAppMenu('window')`.
- **State/data:** **sin persistencia** — [R210] se fueron `dspOnboardV1` y `dspTour_*`. DOM `#tourOv` (captura de clics transparente) → `.hole` (foco por `box-shadow:0 0 0 9999px`, pointer-events:none) + `.card` (`#tourSkip/#tourBack/#tourNext`). `tourSteps(fmt)` = 5 pasos `{sel,title,body}`; `_tourStop` guarda el desmontaje activo; `_tourSkipNext` lo silencia una vez.
- **Key symbols:** `draw()` coloca el agujero sobre `getBoundingClientRect(sel)` (centrado a pantalla completa si `sel:null`), la tarjeta se ajusta debajo/encima del objetivo; teclas Esc/←/→/Enter; `resize` redibuja. Escena de demostración (`buildDemoProject`: texto de título + elipse/rect/línea en V1–V4) **sin llamantes en la app** desde R178 — la usan los arneses de `scratchpad/`.
- **Invariants / gotchas:** `startTour` recibe el FORMATO (`dome|flat|room`), **no** un booleano — cambió en R178. `#tourOv` es **z-45 a propósito**: encima del cromo pero DEBAJO de los `.overlay` (z-50), para que un diálogo de confirmación siga siendo clicable durante el recorrido. `tourTrasCrear` espera 900 ms a que el editor esté pintado (el agujero se mide sobre elementos reales). `newProject` es async y llama a `confirmDiscard` → hay que `await`earlo. Movimiento reducido: sin transición del agujero. **[R210]** `startOnboarding()` (arranque de primera vez con escena de demostración) está archivado desde R178: no queda gancho en `init()`.
- **Status:** ✅ (R145)
