...
| Choosers device+param | 2 chips (Effect-type + Parameter con swatch de color) en la cabecera de pista; `title` por chip combina info + affordance de clic [R215] | app.js · `autoDuo`/`autoDuoText` · `.autoduo.txt .achip` | ✅ | Rev1 §6 |
...
- **Invariants / gotchas:** [R94b/R95·E4] only the focused lane shows the two selects; others render as text. Selects stop pointer/click/dblclick propagation so the track header's select/drag-reorder/rename don't fire. **[R215]** each `.achip`'s `title` now combines the info text with the click affordance (`"${dev} · Click to change"` / `"${par} · Click to change"`) — R214 had set it to just the info text, which fully covered the wrapper `.autoduo.txt`'s own title ("Click to change device / parameter"), so hovering a chip hid the fact that it was clickable.
- **Status:** ✅
- **Roadmap:** —
