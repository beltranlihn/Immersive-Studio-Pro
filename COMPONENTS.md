...
| **Proxy de composición** | Caché de un nest: 1 decodificador en vez de N (**medido 2,6 → 15,8 fps**) | app.js · `ncBuild`/`ncUsable`/`nestSig` | ✅ | R180 |
...
- **Purpose:** `2D|3D` · superposiciones · calidad · `Orbit|Viewer` · FOV · Proxy · zoom · Output.
...
### Recorrido guiado — `tourSteps(fmt)` / `startTour(fmt)` (app.js ~L2768)
- **Purpose:** sale la primera vez que se crea un proyecto de cada FORMATO desde el launcher, con textos propios de domo, 2D o sala.
- **[R178]** Antes saltaba ANTES del launcher, sobre una escena de demostración (`startOnboarding`, archivada). Ahora `init()` va siempre al launcher y `lchCreate()` dispara el recorrido. Banderas por formato: `dspTour_dome` / `dspTour_flat` / `dspTour_room`.
- **Invariants / gotchas:** `startTour` recibe el FORMATO, no un booleano. `buildDemoProject` quedó sin llamantes en la app pero NO se archivó: la usan los arneses de prueba de `scratchpad/`.
- **Status:** ✅
...
- **[R188] Por qué el export va lento — MEDIDO sobre `untitled.isp` (domo 4096²@60, anillo de 6 sobre 2 archivos).** Reparto por fotograma: `seekExport` **80%** (498 ms), comprimir PNG 19% (118 ms), disco 1%, componer nests **0 ms**, composite final **0 ms**. La GPU no hace nada: el export está PARADO esperando a los `<video>`.
  ...
  - **⚠ LA CAUSA DE FONDO, SIN ARREGLAR:** `<video>.currentTime = t` decodifica **desde el fotograma clave anterior** en cada llamada. Como el export avanza secuencialmente, la distancia al fotograma clave crece con cada fotograma y el coste con ella — **medido: 90 ms en el fotograma 1 → ~900 ms en el 24**, y se estanca al llegar al siguiente fotograma clave. Exportar así es O(n²) por GOP. El arreglo de verdad es el **decodificador secuencial de WebCodecs de R108** (`makeClipDecoder`/`demuxMP4`), que ya existe y se midió a ~800 fps en aislamiento — pero `_useCD` lo prohíbe expresamente en export (`exige !_exportQuality`) y `state.view.wcDecode` viene apagado. **Ojo al portarlo:** la rama CD de `vinstSeek` se rinde tras 90 rAF y resuelve SIN fotograma; en reproducción eso es un salto, en un export sería un fotograma equivocado en el máster.
...
- **[R186] RETIRADO DE LA INTERFAZ** (código intacto, interruptor `Comp` intacto). El proxy es ahora **siempre el lienzo completo** (`ncFullSize`; fuera los divisores 1/2, 1/3, 1/4 — Beltrán lo usa siempre a tamaño completo y una opción que nadie toca es una decisión que le cobras al usuario por nada). Pero queda una **discrepancia de encuadre sin cerrar**: en R180 el caso cuadrado medía PSNR 68,7 y 0,03 px de desplazamiento; tras el cambio a lienzo completo mide **PSNR 26,6 y 7 px sobre 32**. Descartados por medición: letterbox (el archivo se hornea cuadrado), volteo de `UNPACK_FLIP_Y_WEBGL` (probado, sin efecto) y fundido de horizonte (`state.view.hfade` es falsy por defecto). Un control contra el clip puesto DIRECTO en la línea de tiempo dio además que **ambos caminos del nest (con y sin caché) difieren del directo**, lo que sugiere que el desajuste puede ser del propio `prepNests` y no del caché — sin confirmar. Con clientes entrando, una previsualización que reencuadra es peor que no tener el acelerador: se reponen las dos entradas de menú cuando la fidelidad esté probada en varias relaciones de aspecto.
- **Status:** ⏸ retirado de la interfaz · el motor sigue verificado en velocidad (2,6→15,8 fps · 6 decodificadores → 1 · bypass en export · invalidación reversible · persistencia)
...
- **[R185] Endurecido para producción.**
  - **La lista de códecs se filtra por el tamaño real** (`exCodecList`, sondeando `pickAvcCodec` contra `exPx()`): un desplegable que ofrece lo imposible le cuesta al cliente un render entero para descubrirlo. Medido en esta máquina: H.264 llega a 4K UHD y 4K DCI pero **no** al cuadrado 4096² de un domo (es cuenta de píxeles, no de ancho). Si el códec elegido deja de caber al cambiar el tamaño, cae a PNG **y se dice**.
...
- **Purpose:** First launch (flag absent) skips the landing, builds a throwaway dome demo with reference shapes, and walks a coach-mark tour of viewport → timeline → inspector → export. Skippable; never reappears; relaunchable from **Window → Guided tour**.
- **Location:** app.js · `startOnboarding()` / `buildDemoProject()` / `startTour()` + `_demoAddShape`/`_demoAddText`, flag helpers `onboardDone()`/`setOnboardDone()` (block right after `showLanding`, ~L2173). Init hook in `showSplash(2,…)` callback (`!onboardDone() ? startOnboarding() : showLanding()`). Menu item in `openAppMenu('window')`.
- **State/data:** localStorage flag `ONBOARD_KEY='dspOnboardV1'`. Tour DOM `#tourOv` (transparent click-catcher) → `.hole` (spotlight via `box-shadow:0 0 0 9999px`, pointer-events:none) + `.card` (`#tourSkip/#tourBack/#tourNext`). `tourSteps()` = 5 steps `{sel,title,body}`; `_tourStop` holds the active teardown.
- **Key symbols:** demo scene = title text + ellipse/rect/line on lanes V1–V4 (`buildDemoProject` awaits `newProject('dome',…)` then places clips, sets dome `props.az/el/size`, `clearAllUndo`+`dirty=false`). Tour: `draw()` positions hole over `getBoundingClientRect(sel)` (centered full-dim when `sel:null`), card clamps below/above target; keys Esc/←/→/Enter; `resize` re-draws.
- **Invariants / gotchas:** flag set only in `end()` (skip/finish) — closing the app mid-tour re-offers it next launch (by design). Relaunch (`startTour`) is non-destructive (no `buildDemoProject`). `newProject` is async and calls `confirmDiscard` → `buildDemoProject`/`startOnboarding` must be `await`ed. `startOnboarding` wraps the build in try/catch → falls back to `showLanding()` so a failure never strands a blank editor. `#tourOv` is **z-45 on purpose** — above app chrome but BELOW `.overlay` dialogs (z-50) so a close-confirm stays clickable during the tour. `startTour(demo)`: `true` (first-run) references the demo scene in the copy, `false`/absent (menu relaunch) uses generic copy true on any project. Reduced-motion drops the hole transition.
- **Status:** ✅ (R145)
...
- **Deuda anotada:** la **sala** sigue con el esquema `drawRoomIso` — `renderRoom3D` necesita `activeSeq().room`, que en el launcher todavía no existe, así que haría falta inyectar una secuencia temporal. El fisheye del domo sigue con `drawSeqViz` a propósito (es un diagrama de cobertura, no un viewport). En la tira cosida, el total se solapa con la etiqueta del último muro (painter compartido con el diálogo de sala). En la tira cosida, el total que dibuja `drawRoomStrip` se solapa con la etiqueta del último muro a este alto (comportamiento del painter compartido con el diálogo de sala; no se tocó para no cambiar el diálogo).
