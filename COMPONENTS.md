...
| Import | Archivos/drag/carpetas/secuencias. **[R245] La detección de SECUENCIA de imágenes (3+ con nombre numerado → pregunta fps) corre SÓLO en el selector de archivos.** El arrastre pasa `opts.noSeq` en las tres llamadas de `importDropped` (carpeta soltada incluida): soltar capturas numeradas abría un diálogo que no dejaba seguir, para una secuencia que nadie pidió. **Ojo al probar:** `importFiles` deduplica por nombre+tamaño, así que dos pruebas seguidas con los mismos nombres dan un falso «no preguntó» | app.js · `importFiles(files,folder,opts)`/`importDropped` (~L2294) | ✅ | R245 |
