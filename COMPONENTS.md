...
| Gesto de mover | Move/copy de clip con ghost. Partner A/V enlazado: sólo horizontal (`primaryIds` vs `items`). **[R231]** la pista destino se valida por la PISTA del clip (`isAudioClip`), no por el medio. **[R349b] `cutOverlapsOnDrop` va DENTRO de `if(changed)`**: es destructivo (recorta o elimina al vecino solapado) y la foto de deshacer cuelga de esa misma condición, así que un gesto que no mueve nada lo ejecutaba **sin historial** — medido, un clic con 3 px de temblor sobre un clip que solapaba a otro recortaba al otro de `start 2, dur 4` a `start 4, dur 2`, la pila no crecía (2→2) y el Ctrl+Z siguiente BORRABA el clip. Existía para el clic de 0 px exactos; el umbral de 6 px lo abrió a todo clic con temblor. **[R349] umbral de arrastre `TL_UMBRAL`=6 px**: hasta que el puntero no se aleja esa distancia, `onTLMove` no hace nada. No había ninguno — seleccionar y mover eran el mismo gesto desde el primer píxel, así que el temblor de la mano al hacer clic ya desplazaba el clip, y con la línea apretada un píxel son varios fotogramas. Mismo orden que los umbrales que el editor de curvas (3-4 px) y el arrastre de carpetas (5 px) ya llevaban a mano | app.js · `onTLMove`/`onTLUp` · `TL_UMBRAL` | ✅ | R349 |
...
| Trim contextual | ripple/roll/slip/slide (T); espeja el recorte en el partner enlazado. **[R322]** el ripple acota el gesto por el clip MÁS A LA IZQUIERDA de los que arrastra (desde R321 incluye mitades enlazadas de otra pista, que pueden estar en otro sitio) y al SOLTAR resuelve los solapes con `cutOverlapsOnDrop`, como el arrastre de clip. **[R313·A10 → R314]** rebasa la automatización cuando el MATERIAL se mueve. La instantánea se toma DENTRO de `applyTrim` (`capAuto`, perezosa y cacheada en `base`), no en el llamador: así queda cubierto también `trimNudge` —el mismo recorte por TECLADO, que en R313 era un no-op— y no se puede invocar sin ella. Y el rebase del PARTNER enlazado vive en `_mirrorLinkTrim`. **[R320]** aquel «punto único por el que pasan los cinco modos» no lo era —son SIETE llamadas— y faltaban tres cosas: `slip` no rebasaba el clip agarrado (su mitad enlazada SÍ, así que un par A/V quedaba con vídeo y audio desalineados), `slide` no rebasaba a su vecino de la derecha, y `slide` y el crossfade llamaban sin instantánea. Deuda anotada: `slide` no captura `pLinkBase`/`nLinkBase`, así que las mitades enlazadas de sus vecinos no se mueven con ellos (el roll sí las captura). | app.js · `trimZone`/`applyTrim`/`capAuto`/`_mirrorLinkTrim`/`rebaseAutoPorMaterial` | ✅ | R314 |
| Trim por handle | `.hd.l`/`.hd.r` resize. **[R322]** las instantáneas del gesto usan `capAuto`, no `JSON.stringify(x||{})`: el literal vacío es truthy y hacía que el rebase delegado escribiera `kf:{}` en clips sin automatización, y `serClip` lo guardaba en el `.isp`. **[R321]** el clamp del arrastre divide `inP0` por `speed` —está en segundos de FUENTE, y sin dividir un clip a 0,5× paraba a mitad del material que tiene detrás—. **[R320]** el rebase de keyframes ya NO está duplicado aquí: `trimItem` delega en `rebaseAutoPorMaterial`, que nació como copia suya en R313 y había recibido dos arreglos que el original nunca vio (sin `src.length>1`, saltando entradas vacías). | app.js · `trimItem` / drag.trimL/R | ✅ | [T2] |
...
| Selección temporal & marquee | Selección por span/rect → loop. **[R350] los clips se marcan MIENTRAS se arrastra**, no al soltar (petición de Beltrán: «que se sienta realtime»). El cálculo ya existía —era la primera línea de la rama `else` de `up`—; se extrae a `idsEnRango()`, **una sola implementación** que usan el movimiento y el soltar. En vivo van el resaltado y el contador de la barra de estado; `renderInspector()` se queda en el `pointerup`, porque rehace el panel entero y durante el barrido parpadearía por cinco clips distintos. Se repinta **sólo cuando el conjunto cambia** (firma `ids.join(',')`), o sea unas pocas veces por gesto al cruzar el borde de un clip, no en cada `pointermove`. La caché de firma es segura porque nada más toca esas clases a mitad de gesto, y `renderTimeline` las repone desde `state.selIds` si reconstruyera los nodos | app.js · `startTimeSelect` (`idsEnRango`/`pintarSel`) · `startMarquee` · red `scratchpad/r350-seleccion-viva.mjs` | ✅ | R350 |
...
| `makeProxy` | Proxy all-intra GOP=1 + m.frames. **[R241] su tasa sale de `m.fps`** → si `detectFps` falla, el proxy nace a media tasa (medido: 455 de 911 fotogramas) y el montaje enseña un fotograma de cada dos. **[R347·b] y si `m.fps` está CANONIZADA, el proxy sale con OTRA cadencia**: `detectFps` redondeaba 59,94 a 60 y de `m.fps` salen `total=Math.round(dur*fps)` y `us=1e6/fps`, así que `drawEncFrame` sella cada fotograma en `Math.round(idx*us)` — el proxy queda escrito a 60 lo que es 59,94, o sea un 0,1 % rápido: ~36 fotogramas de desfase al final de un clip de 10 min, y `total` 35964 contra 36000, con el último medio segundo sin fotogramas. **Ése es el mecanismo que muerde en el camino por defecto** (el rápido de WebCodecs), y R347 lo arregló sin medirlo ni escribirlo. **Lo que sí se midió** —al 60 % de un archivo NTSC de 30 s, 40 de 40 fotogramas desplazados— es la rejilla `i/fps` sobre el ClipDecoder y `<video>`, o sea un **MODELO** del tercer camino (el de seek), no una medida de un proxy generado: la misma sustitución que R346c señaló doce líneas más abajo, así que se deja etiquetada como lo que es. **Y el desplazamiento no empieza «a los 8 s»**: esos `1001/(2·fps)` s son el número del CENTRO del fotograma; sobre la rejilla de FRONTERA que este camino pide, una cadencia equivocada desplaza ya el fotograma 1, y a los `1001/fps` s pasa a dos. Arreglado en `detectFps`; red `scratchpad/r347-ntsc.mjs`. **[R346b·c]** su camino de seek pide la rejilla `i/fps`, o sea la frontera exacta de cada fotograma, y aquí el fotograma equivocado se HORNEA en el archivo: la tolerancia la aplica ahora `seekRaw`. Es el TERCER camino (el rápido de WebCodecs va por índice y es inmune, el de rVFC se pilota por `mediaTime`), así que sólo muerde cuando el demuxador o el códec fallan | app.js · `makeProxy` (~L3266) · `seekRaw` (~L3245) | ✅ | [C3] · R346c · R347 |
| `detectFps` (~L3114) | Tasa real del vídeo. **[R347b] DOS estimadores, y cada uno responde sólo lo que su dato permite.** El fino es el **demuxador** (`(sc-1)/durSec`, promedia el archivo entero y cancela el redondeo del timebase: medido 24,000669 / 60,001669 / 29,969113 / 23,976608, las cuatro del lado correcto) y es el único que decide entre NTSC y entero. El basto es la **reproducción** por `requestVideoFrameCallback`, para lo que el demuxador no lee (WebM/MKV/AVI, moov ilegible), y vuelve a **redondear a entero** antes de canonizar. **Por qué:** R347 le pidió a la medida por rVFC que distinguiera el 0,1 % que separa 59,94 de 60 y no puede — `mediaTime` es el pts EN EL TIMEBASE DEL CONTENEDOR, y Matroska lo fija en milisegundos (que `ffmpeg -c copy` arrastra al MP4), así que un 24 EXACTO da intervalos 42,41,42,42,41… ms, mediana 42, `1/md` = 23,8095, **más cerca de 23,976 que de 24**. Lo decisivo: en ese timebase un 24 exacto y un 23,976 dan la MISMA mediana, así que quitar el redondeo no afinó nada, sólo hizo que adivinara — medido en la app con WebM reales, 24 → 23,976 y 60 → 59,94, el fallo de R347 con el signo cambiado sobre material corriente. El redondeo devuelve además su valor exacto a las cadencias no canónicas (12/15/16 fps daban 12,048 / 14,925 / 15,873) y quita la dependencia del orden en el camino del plazo vencido. El juego fino lleva las **cinco** NTSC (faltaban 119,88 —la de deriva más rápida, medio fotograma en 4,2 s— y 47,952) y en cada pareja el **entero va primero**, para que un empate caiga del lado seguro. **Y de rebote, un fallo mayor que el de NTSC:** el `canon` anterior devolvía el PRIMER candidato dentro de 1,2 y 24 precede a 25, así que `|25−24| = 1` disparaba antes de probar el 25 y **todo material de 25p se detectaba como 24 — un 4 % de error, cuarenta veces el de NTSC, desde el commit inicial**; con `fps=24` el proxy de un 25p de 10 min sólo contenía los primeros 9,6 minutos estirados sobre 10, y la comprobación de duración de `bindProxyFile` lo daba por bueno. Cubierto ya por `pal-25.mp4` en la red. **[R347b] Y los proxies del régimen viejo:** `proxyRevisaFps` (~L3145) suelta el proxy y avisa cuando se horneó con otra cadencia — se compara con `m.proxyFps`, que ahora se guarda al generar y viaja en el `.isp`; si falta, el proxy es anterior y sólo es sospechoso cuando la cadencia real no es entera. **[R241]** al vencer el plazo YA NO tira las muestras (tres intervalos bastan para la mediana) y el plazo escala con los megapíxeles hasta 8 s: con 2,5 s fijos, un HEVC de 7196×912 a 410 Mbps no llegaba a diez fotogramas y se quedaba en el 30 por defecto — **0 de 9 aciertos en el material real de Beltrán, ahora 9 de 9**. **[R347 → corregido en R347b]** R347 quitó el `Math.round(1/md)` para que la mediana entrara en `canon` sin redondear, y escribió que «`mediaTime` es el pts exacto, así que la mediana vale 1001/30000 clavado»: **eso es falso** para cualquier contenedor cuyo timebase no sea múltiplo del fotograma, y es exactamente donde el criterio se equivocaba. Lo que aguanta de R347: que `m.fps` manda en la rejilla del proxy, y que las tasas NTSC tienen que ser distinguibles. Cómo se distinguen ahora está arriba. Red en los dos sentidos: `r347-ntsc.mjs`, con **ocho** archivos — las tres NTSC, dos de timebase de milisegundos (24 y 60 exactos: el caso que R347 rompía), el PAL de 25, y dos de timebase fino | app.js · `detectFps` (~L3114) · `proxyRevisaFps` (~L3145) | ✅ | R241 · R347b |
...
| `demuxMP4` | Demuxer por rango (moov+samples) | app.js · `demuxMP4` (~L3978) | ✅ | R108 | **[R342]** aplica la EDIT LIST (`edts/elst`): `media_time` en unidades del medio y los huecos vacíos en las de la película (de ahí el `mvhd`). Sin ella, cualquier archivo recortado sin recodificar salía corrido entre la previsualización (`<video>`, que sí la respeta) y el export; devuelve `elstOff` para poder afirmarlo en una sonda.
| ClipDecoder | Anillo WebCodecs decode-ahead. **[R256] Sale solo del bloqueo mutuo** que producía un salto hacia atrás corto (un clip en bucle, cada vuelta): los fotogramas retenidos por delante del destino agotan el fondo de salida del decodificador y `evict` no los suelta, así que ni se alimenta ni se decoda. Antes aguantaba 10 s, se rendía y el export caía a `<video>` (914 → 90 ms/fotograma) | app.js · `makeClipDecoder` (~L8069) · la salida del atasco en `step()` — **[R259] la condición es LÓGICA** (el fotograma pedido es más viejo que TODO lo cacheado → no puede llegar), no un contador de vueltas: contar vueltas disparaba sobre exportaciones sanas de 3+ capas y la tormenta de reinicios tiraba el contexto gráfico · **[R261] el vaciado lleva la generación del decodificador** (el `flush()` que `close()` rechaza ya no marca como vaciado al que acaba de nacer) · **[R344] `resetTo` deja `lastFedPts` en `null`, no en `-1`**: el `-1` que R343 se dejó ahí hacía que la rama de salto grande disparase en cada vuelta tras cada reinicio — 10 `VideoDecoder` recreados por salto, medido | ✅ export · 🚧 preview | R189 · R256 · R259 · R261 · R344 · [C2] |
| vinst + servo | `<video>`+tex por clip, servo de velocidad. **[R330]** la instancia se guarda por `c.id`, pero la FUENTE de un clip puede cambiar sin cambiar de clip (el tejido barajado rota `mediaEfId`): `vi.mid` detecta el cambio, tira el decodificador viejo, desata el `<video>` y cierra la carrera de un demux en vuelo del medio anterior. **[R346c] el tope de instancias se sube por LOTE (`vinstCapPara`) en los TRES sitios que abanican `vinstSeek` y lo esperan entero** —`seekExport`, `scrubRender` y `raPrerenderRange`—: R189 lo escribió sólo en el primero, y con más de `VINST_MAX` clips dibujados el propio abanico desaloja las instancias que está esperando (su `seeked` no llega nunca porque `vinstDispose` hace `removeAttribute('src')`) → el `Promise.all` no resuelve y la previsualización se congela sin error. **[R346c] la vuelta del bucle** (`ploop`) recoloca cada clip dibujado en su tiempo local de `workIn`, no `m.el` a 0 —que es el elemento por MEDIO, del que la previsualización ya no dibuja— y abre la puerta del servo (`_seekT=0`) | app.js · `vinstEnsure`/`ploop`/`vinstCapPara` | ✅ | R330 · R346c |
...
| **Estética de la sección Motion** [R349] | Un modificador se pinta como un EFECTO: mismas clases (`fxcard`/`fxhdr`/`fxbody`/`prow`), pestaña de color a la izquierda con el tono de `autoColor` sobre la MISMA clave que usa su curva en la línea de tiempo (`mot:<param>:mix`) —así se reconoce por color en los dos sitios, igual que los efectos desde R270—, botón de encendido con el icono `power`, papelera con `trash`, y el Mix como una `.prow` con su fader teñido, el valor en `.box` y el rombo en `.nav`. Era la única zona del inspector con lenguaje propio (fondo `#1b1e24` a mano, encendido como un punto redondo, la papelera escrita `×`, la cifra suelta al lado). **Modo, velocidad y amplitud bajan de la cabecera al cuerpo**, cada uno en su fila con etiqueta: en cabecera eran cinco controles en 280 px y salían truncados («Slide ↔ (do…»). La fila de amplitud se oculta entera fuera del modo Onda (el input sigue en el DOM: `refreshAnimAmps` lo busca). Señales de automatización unificadas con el resto del inspector: `.prow.auto` para «lleva curva» y `.kf.on` para «el cabezal está sobre un punto» — se acabó el `◆` de texto tras la cifra. **[R349b]** las filas sin rombo (Modo, Velocidad, Amplitud y la Intensidad del maestro) reservan su hueco con un `.nav` de ancho EXPLÍCITO (20 px, el de `.prow .nav button`), no con un `.kf` vacío: la regla `.prow .kf:not([data-kf]){display:none}` de R143 esconde ese relleno y el `.nav` se quedaría a cero — que es por lo que las filas sin diamante de `fxFaderRow` (Ataque, Caída, Curva, Rebote) tampoco alinean hoy. El maestro de acorde (Flotar) sigue el mismo patrón. **Gotcha:** `.animon`/`.animdel` tienen que estar en la regla `.fxhdr .fxtog,…{background:none}` de `index.html` o heredan el fondo por defecto del `<button>` y salen como dos recuadros claros | app.js · `buildAnimList` · `refreshMotionWet` / `refreshAnimAmps` · index.html regla `.fxhdr` · volcado `scratchpad/r349-motion.mjs` | ✅ | R349 |
| **Intensidad vs Reactividad** [R349] | No son el mismo mando y se leían como si lo fueran (lo preguntó Beltrán). `fxIntensity` es literalmente `clamp01(int/100 + amt/100 × fxModLevel(fx))`: **Intensidad** es el suelo siempre presente, independiente del audio; **Reactividad** es cuánto SUMA el audio por encima (no lo sustituye). La cuenta va escrita bajo «Respuesta» en la tarjeta reactiva, y `fxFaderRow` admite un `tip` opcional que se pone como `title` de la fila entera | app.js · `fxIntensity` · `fxFaderRow(…,tip)` · `fxCardHtml` (rama reactiva y rama Motion) | ✅ | R349 |
...
| **Play fuera del tramo** [R279 · R286b · R349·b] | `_enBucle` (¿envuelve? sólo si el cabezal cae DENTRO del tramo) se decide **al arrancar**; la PARADA en `workOut`, **no**: se mira si el cabezal CRUZA `workOut` hacia adelante en ese fotograma (`phPrev<workOut<=playhead`, con `phPrev` tomado al entrar en `ploop`, antes de avanzarlo). R286b repuso la parada mirando sólo `playhead>=workOut` — cierto desde el primer fotograma cuando se da al play DETRÁS del tramo, que es donde suele quedar el cabezal tras repasar el final: volvía a `workOut` y pausaba en el acto. **[R349b]** el primer arreglo lo resolvió con una bandera decidida en `play()` (`_paraEnOut`) y estrenó el simétrico: marcar entrada/salida MIENTRAS suena (teclas I/O, Ctrl+L, arrastre de la llave del bucle — los tres vivos durante la reproducción) dejaba la bandera en falso y **la reproducción no se detenía nunca**, la avería literal del comentario de R286b (medido: con `workOut=1`, a los 2,5 s el cabezal iba por 3,5). El cruce distingue los tres casos sin guardar estado. **No sirve `_phLast`**: sobrevive a la pausa, así que en el primer fotograma tras un play trae el valor de la reproducción anterior | app.js · `play()` / `ploop()` · `_enBucle` · `phPrev` | ✅ | R349b |
| **Tramo del bucle** [R250] | El motor envuelve desde R81 sobre `[inP, inP+loopLen)` y `loopLen` se captura de la duración del clip AL ENCENDER el bucle — o sea que recortar primero (o venir del monitor de origen con marcas) ya daba un bucle corto. **No faltaba función, faltaba verla**: el inspector muestra ahora la longitud del ciclo (con decimales: `fmtDur` redondea y un bucle de 6,5 s se leía «6s»), qué trozo de la fuente repite en código de tiempo, y un botón **«Del clip»**. `setLoopRange` cambia SÓLO `loopLen`: antes había que apagar el bucle para re-decidirlo, y apagarlo RECORTA el clip a lo que quede de archivo (40 s → 30,13 s medidos), así que re-decidir costaba el montaje | app.js · `setLoopRange` · `_renderInspectorMain` (filas de Playback) · `srcT` (**[R256] el ciclo ya cierra**: el módulo en coma flotante repetía el último fotograma en una de cada tres vueltas, y el mismo punto del ciclo salía como 4,2 o 4,199999999999999 — que `keyForTime` trunca a DOS fotogramas distintos) / `_applyLoopToggle` · **[R346c] el espejo del ping-pong (`ph=L-ph`) NO SE TOCA**: R346b lo topó con `Math.min(L-ph, L-2*TOL_DECOD)` para que el vértice no cayera en el extremo excluido de la ventana, y salió peor — `Math.min` no corre la rampa, **aplana el vértice sobre su vecino**, y el giro pasaba de `22,23,24,23,22` a `22,23,23,23,22`: una meseta de tres fotogramas horneada en el export. Barrido de 4000 configuraciones: 604 empeoradas contra 346 mejoradas, 0 en las que el tope quitara una congelación, y **ningún tamaño de tope funciona** (4 µs, 1 ms, medio fotograma y un fotograma dan la misma meseta). Cerrar la ventana de verdad exige restar un PASO DE FOTOGRAMA con el fps de la FUENTE. Red: `scratchpad/r346c-pingpong.mjs` | ✅ | R250 · R256 · R346c |
...
- **Location:** app.js · `FSEQ` (~L389) · `FSPH`/`PSPH`/`buildSphereMesh`/`equirectClipAt` (**[R329]** lee de `clipsVisibles`: apagar el clip o silenciar su pista lo quitaba del domo pero no de la esfera)/`drawEquirectSphere` (~L457) · llamada dentro del bloque 3D de `render()` · `pareceEquirect(m)` junto a `makeClip`.
...
- **[R349] La colocación acota los DOS ejes y sabe recortarse.** Antes sólo se miraba el borde de abajo (`if(bottom>innerHeight) top = y - alto`), que con un menú alto da un `top` NEGATIVO: el de «Añadir efecto» son 25 efectos + 4 separadores ≈ 720 px y se abría con la cabeza fuera de la pantalla. Además `openFxMenu` ADIVINABA la altura para anclarlo por encima del botón (`min(items,20)*22`, sin contar separadores) y remataba con un `Math.max(46,…)`. Ahora el ancla es el propio botón (`r.bottom+4`) y `openMenu` decide: si no cabe de alto le pone `maxHeight` + barra propia, y luego acota `left`/`top` a un mínimo de 4 px y a que el borde opuesto entre. Medido barriendo veinte alturas de botón: **6 de 20 se salían con la colocación anterior** (la peor a −528 px), 0 de 20 con la de ahora (`scratchpad/r349-revision.mjs`, que reaplica la colocación vieja sobre el mismo nodo para verse roja).
...
- **Purpose:** Shared full-screen-quad vertex shader + compile helper + VAO for the ping-pong FBO post/FX chain. **[R337 → R338]** los destinos del ping-pong (`_ppRT`) se guardan **por TAMAÑO y con tope** (`FXRT_MAX`, desalojo por menos-usado: R337 quitó el churn pero dejó retención ilimitada — mover la calidad de previsualización retenía ~400 MB). Los medios búferes del **bloom** comparten ese mismo pool con ranuras `'b0'`/`'b1'`: eran la pareja única que R337 no barrió: hay dos cadenas con tamaños distintos en el MISMO fotograma — la de un clip a `fxChainSize()` (1280 en previsualización) y la de una capa de ajuste al tamaño del composite (2048 en domo)— y con una pareja única cada fotograma hacía cuatro `texImage2D` de reasignación, dos de ellas de 2048² RGBA. `freeFxResources` vacía el mapa entero. Note: the FX/post fragment shaders themselves (fisheye pre-warp, keyer, bloom, FXTYPES, mix, DXT-for-HAP) belong to other subsystems; listed here only as the GL infra they share.
...
- **Key symbols:** **[R237]** arranca llamando a `syncCompSize()` — el máster lleva la FORMA de lo que se va a dibujar, y de ahí depende el viewport de relleno. Luego fija el contexto de la secuencia con **[R330]** `ctxCompMaster()` (`_drawFlat`, `_roomWrap`, `_compAspect` y `_zsortSize` juntos: los tres sitios de arriba lo montaban a mano y ninguno recordaba el cuarto) + `_arTime`. Composite step: `raGet(playhead)` cache hit → reuse `_raHit`; else `prepNests()` + `composite(t,null,false,true)` (con RELLENO) into `compFBO` + `raStore()`. Branch: `mode==='3d'&&isRoom()`→`renderRoom3D()` (L930); `mode==='3d'&&!flat`→dome mesh program `P3`/`domeVAO`, `cameraMVP`, `buildDomeMesh(curCovHalf())` (L931); else→2D blit program `PB`/`quadVAO` + `drawGrid2D()` (L943). Pop-out viewer via `renderViewer(_srcTex)` (L955). **[R332]** `_reuseComp` reutiliza `_lastSrcTex` — pero AHORA se comprueba (mismo fotograma, misma generación y, si salió del adelanto, que la caché siga guardándola: `raEsSuTex`). El adelanto recicla texturas por un pool, así que reutilizar el puntero a ciegas estampaba un fotograma de otro tiempo en el visor **y en el editor** (el repintado de restauración).
...
- **[R341 → R343] `chapaLienzo` y el recorte al círculo:** en modo domo devuelve su propio lienzo con lo de fuera del disco a alfa 0 — correcto para una ENTREGA. **Excepción:** el horneado del caché de un nido (`_ncSquare`, la misma bandera que usan las otras excepciones del mismo hecho) NO se recorta: ese archivo es una textura intermedia que el padre muestrea entera, y recortarla hacía que activar el caché cambiara la imagen. `lienzoPng` pregunta ahora `q===glc` en vez de suponer que en domo siempre hay lienzo propio.
- **[R339 → R343] `drawComposePreview` devuelve el reparto REAL** (`lay.length`) por sus cinco salidas, y el rótulo `#cPrevLbl` lo usa en vez de `g.count`, que es lo pedido. La rama del esquema se elige como la elige `createComposition` (`weave || isFlat()`), no sólo por el tipo: `grid`/`row`/`col`/`random` existen en las dos listas y en domo se pintaba el esquema plano.
- **Purpose:** Draws all active clips for time `t` into the currently-bound FBO at `size²`. `compositeClips()` resolves per-lane clip stacking + cross-dissolve transitions, y es la **única** fuente de «qué se ve». **[R329]** había otras dos: `activeClips()` — que además estaba REESCRITA EN CALIENTE 3900 líneas más abajo para añadirle el mute— alimentándo a los buscadores del visor sin mirar `disabled`, y el recorrido propio de `equirectClipAt`. Las tres son ahora `clipsVisibles(t)` = `compositeClips(t)`: lo que se puede señalar es exactamente lo que se está pintando.
- **Location:** app.js · `composite(t,size,opaque)` · `compositeClips(t)` · **[R329]** `clipsVisibles(t)` (sustituye a `activeClips`).
...
- **Key symbols:** `isFlat()` = seqMode 'flat' OR 'room' (room composites as a rectangular unwrapped strip); `isRoom()` = 'room' only; `flatLikeMode` for export dims / format chip / nested-sequence draw. Globals set per-composite: `_drawFlat`, `_compAspect`, `_roomWrap`, `_zsortSize` y **[R330]** `state.seqCov` — los pone `ctxCompMaster()` para el máster y `prepNests` (con save/restore) para cada nido. La cobertura era el único que el nido no cambiaba: uno de 220° dentro de un padre de 180° deformaba sus clips con la del padre.
...
- **Key symbols:** classes `.lanehdr .sel .collapsed .aud`; buttons `[data-m=collapse|mute|solo]`, `.laneres [data-m=resize]`. `laneH(li)`. Constantes **del diseño (R152)**: `LANE_DEF_H=57, LANE_MIN_H=26, LANE_MAX_H=120, LANE_COLLAPSED_H=24`, `AUDIO_LANE_H=LANE_DEF_H` (**[R171]** el audio mide lo MISMO que el vídeo; sigue siendo sólo el valor por defecto), `RULER_H=24`, **[R231] `LANE_COL`/`laneColor(lane)`** (sustituyen al rotativo `TRACK_COLORS`). **[R163]** `AUTO_LANE_MIN_H=52` = suelo en modo automatización para pistas de vídeo (los desplegables de identidad necesitan margen); `laneFloorH(l)` es la fuente única y la usan `laneH`, `wheelResizeLanes` y el arrastre de la barra vertical. **[R352] `laneAltaActual(kind)` / `laneAplicaAlta(nl)`**: una pista NUEVA nace con el alto que tienen las que ya están, no con `LANE_DEF_H`. Se creaban sin `h` y, como Alt+rueda redimensiona TODAS a la vez, la recién añadida era siempre la que rompía la fila. Se aplica al objeto de pista ya construido, justo antes de insertarlo, así que el alto no depende del camino — y son **DIEZ**: `addLane` (⌘T y el menú), `ripPlaceOnNewTrack`, `addAdjustmentLayer`, la pista de audio que se fabrica al importar un par A/V, las dos que se crean al arrastrar un medio sin pista de su clase, la de un proyecto viejo sin audio al cargarlo, la de llevar un clip a una pista de su tipo, `laneLibreCerca` y `ensureVideoLanes`. **MEDIANA, no media** (una pista dejada estirada a propósito no debe arrastrar el valor), acotada al suelo de SU clase, y `collapsed` sólo se hereda si están plegadas TODAS. Quedan fuera a propósito `defLanes`/`roomDefLanes` y las pistas internas de una composición: ahí el alto de fábrica ES el correcto porque no hay nada previo con lo que igualarse. Red: `scratchpad/r352-alto-pista-nueva.mjs` (mide el alto RENDERIZADO, no `h`). Por debajo del suelo la pista NO se queda a medias: `collapsed=true`. `lanesTopDown()` display order.
...
- **Location:** app.js · `onTLMove()` (L2427–2456), `onTLUp()` (L2473–2483), `showMoveGhosts()` (L2416), `clearMoveGhosts()` (L2415), `duplicateClipAt()` (L2424). **[R329]** si el Alt-arrastre copia LAS DOS mitades de un par A/V, `onTLUp` las vuelve a enlazar entre sí con un `link` NUEVO (`duplicateClipAt` las hace sueltas a propósito, pero dejarlas así rompía el par: dejaban de moverse y borrarse juntas y no había forma de reenlazarlas).
...
- **Invariants / gotchas:** `movedIds` = lo que acaba de moverse/crecer; esos clips **no se cortan entre sí**, sólo cortan a los quietos. **[R321]** la lista de la pista se rehace por cada clip soltado: estaba calculada UNA vez por pista y los RESTOS que `razorCore` crea al partir un clip no entraban en ella, así que soltando dos a la vez el segundo no veía el resto del primero y el solape sobrevivía. `_cutEdgeTo` delega en **`trimItem`** a propósito: así hereda gratis los límites de origen y el rebase de keyframes con keyframe de frontera (recortar a mano `start/inP` descolgaba la automatización del material). Un resto menor que `CUT_MIN` (50 ms) se elimina en vez de dejar un muñón de dos frames que además seguiría solapando. El resto derecho de un split nace **suelto** (`delete link/avRole`): tres clips con el mismo `link` harían que `linkPartner` eligiera al azar. `_dropClip` también limpia el `link` del superviviente. **Nunca se llama desde `startFadeDrag`** — ahí el solape es intencionado.
- **Status:** ✅
- **Ticket:** [R223] Etapa 1
...
- **Key symbols:** zones `roll|rippleL|rippleR|slip|slide`; `EDGE=12`px. **[R321] `_rippleAfter(c,kind)`** = qué arrastra un ripple: los clips de la pista desde el borde **y las mitades enlazadas de esos clips**, que viven en otra pista y faltaban (todos los pares aguas abajo quedaban desfasados justo el recorte). Un solo punto para el arrastre y para `trimNudge`, que en R313 ya divergieron. Keyboard: Trim tool + ←/→ nudges the edge nearest playhead (Shift=10f, L5756).
...
- **[R323] `rippleDelete` (⇧⌫) — su alcance es SU PISTA, no el proyecto.** Se lleva el clip y su mitad enlazada, y cierra el hueco moviendo lo que `_rippleAfter` dice —la misma función que usa el ripple de la herramienta T— en las pistas de las dos mitades, con las mitades enlazadas de lo que arrastra. **Un solo desplazamiento** para todos (R321 usó uno por pista y con un J-cut descuadraba la pista de audio), y el suelo **por clip** —[R324], porque acotar el desplazamiento común por el clip más a la izquierda dejaba que uno suelto cerca del origen colapsara el gesto entero: medido, un bloque de 10 s cerraba 1 s—. R322 lo reescribió con un modelo de bloque y **perdió el filtro de pista**: medido, borrar un clip de vídeo movía clips sin relación de la pista de audio y de una tercera pista. La liberación de recursos va por `_quitarClips`→`_soltarRecursosClips`, compartida con `deleteSel` y `migrateRoomFloor`: es EL punto por el que salen los clips, así que ahí viven todos — `maskTex`, búferes de realimentación de FX, la selección de puntos y, **[R332]**, `_curveTex` (la rampa de las curvas de color, que nunca pasaba por `deleteTexture`) y la caché de bandas si el clip borrado era su fuente (`_arCache.clip` guarda el OBJETO: los FX reactivos seguían leyendo el `start`/`dur` de un clip que ya no está en la línea de tiempo).
...
- **Location:** app.js · `trackKeyFor`, **`revealAutomation`** (~L5704), `focusAutoParam`, `showAutomationParam`, `clipArmedTrackKeys`; hooks in `buildRows` (field pointerdown / box dblclick / box wheel / diamond click / row contextmenu), `wireFxCards` (`.fxrow` pointerdown / dblclick / kf click / contextmenu) and `buildAnimList` (Mix slider + `.awetkf`). **[R329]** la curva de Mix se guarda por PARÁMETRO (`mot:<param>:mix`) y dos Motion sobre el mismo parámetro la comparten: `soltarMixSiHuerfana(c,param)` es la única implementación de «soltarla si ya no la reclama nadie» (borrar el Motion), y cambiar el parámetro la MUEVE con él en vez de dejarla huérfana e invisible
...
- **Invariants / gotchas:** Deterministic in export (everything derives from `t`, never wall-clock). · **[R322] El acierto de `_modAudioCache` se consulta ANTES de llamar a `specRangeRaw`**: al revés, cada acierto —uno por modulador y por fotograma— pagaba igualmente la pasada de ganancia/puerta que R320 movió dentro de esa función (medido 0,445 ms con una pista de 75 min, tirados a la basura). Es correcto consultarla primero porque `arRecompute()` vacía la caché en cada movimiento de esos dos mandos. · **[R318→R320] las cachés de este camino son tres y se invalidan distinto.** `_modAudioCache` (envolvente moldeada por banda+atk+rel) deriva de `_arCache` y muere con él —se vacía junto a `_fxEnvCache` en los seis puntos de invalidación, siempre las dos juntas—. `m._specRaw` (rango de frecuencia a medida) cachea **la media cruda de los bins, que NO depende de Gain ni de Gate**, con una entrada por rango; puerta y ganancia se aplican después en una pasada lineal sobre `m._specOut`. R318 lo había «arreglado» metiendo los dos mandos en la CLAVE, y el fader de Gain —que recorre 0..300 de uno en uno— reintegraba el espectro entero en cada píxel del arrastre. Ninguna de las dos se serializa: `serMedia` es lista blanca. · [R95·D4] `m.frz` freezes a layer's output. Audio band names must match `computeBands` (`bass|mid|treble|bright`). `evalP` stays untouched — the stack rides on top only in `evalR`. · **[R335] el selector de espectro llegaba a 12 kHz con un análisis a 16 kHz: POR ENCIMA DE NYQUIST.** El tramo alto no enseñaba energía de 12 kHz sino la de ~4 kHz replegada — elegir el charles daba una señal que reacciona a la caja. `SPEC_F1=SPEC_SR/2`. Subirlo de verdad pide subir `SPEC_SR`, y eso empeora la resolución de graves (15,6 → 31,2 Hz por bin), que es justo lo que la decimación a 16 kHz vino a arreglar. · **[R335]** `fxModLevel` devolvía 0 sin pasar por la inversión sólo cuando no hay banda elegida; los otros tres «sin fuente» (sin caché, fuera del clip, sin envolvente) llegaban a `fxShape`, que con **INV** convierte el 0 en 1 — un efecto invertido se disparaba al 100 % justo donde no hay información (antes y después de la canción, o durante un export entero sin bandas).
- **Status:** ✅
- **Roadmap:** [R95·C1]
...
Source: `app.js` (~16164 lines). Line numbers verified against the working tree on 2026-08-10 (R347b) para las filas que esa ronda tocó (`detectFps`, `makeProxy`, `srcT`, monitor de origen, `vinst`); el resto arrastra la verificación de 2026-08-09 (R345b) y puede estar corrido unos cientos de líneas.
...
- **Key symbols:** codec options `png|ffh264|ffhevc|hap|hapq|still` — **[R349]** `mp4` (H.264 por WebCodecs, tope medido 3072²) y `hevc` (H.265 por WebCodecs, ~1080p) **ya no se ofrecen**: las dos filas de FFmpeg que van encima hacen lo mismo hasta 4096 y 8192, y lo único que aportaban era ofrecer un formato que se rinde a la resolución de trabajo y gastarle al usuario un render entero para descubrirlo (decisión de Beltrán). **El código que los exporta NO se retira** — de él dependen *Render in place* (`ripCodecOptions`→`choice.kind`) y el horneado de proxys de composición (`ncBuild`), que siguen pasando `codec:'mp4'|'hevc'` a `runExport`. Una memoria de sesión (`lastExportGet`) o un preajuste guardado con esos valores se traduce con **`exCodecNorm`** (`mp4→ffh264`, `hevc→ffhevc`): sin eso `S.codec` quedaba con un valor que el desplegable no contiene, `sel.value` caía a vacío, `filas.find` no encontraba fila y `S.codecOk` se quedaba en `true` por el `!mio`, así que Exportar seguía pulsable con un formato invisible. **Consecuencia asumida:** en una máquina SIN FFmpeg ya no hay MP4 de respaldo (quedan PNG/HAP). Efecto lateral: `exCabeCodec`/`exTopeCodec`/`EX_LADDER` sólo se llaman desde la rama `c.kind` de `exCodecList`, que hoy no la toma ninguna fila — se conservan porque son el sondeo genérico de un códec de WebCodecs y el panel puede volver a ofrecer uno. **[R349b] `exEstBytes` tiene su propia rama para los dos códecs por FFmpeg** (`Math.max(5,Math.round(S.ffbr||200))`, el mismo acotado que hace `runExport`): caía al `S.br` genérico, que NO es lo que se codifica —`ffq.mbps = S.ffbr`— y cuyo control `#exBrRow` está además oculto para ellos porque `isVid` sólo mira 'mp4'/'hevc'. Medido con los valores de fábrica, domo 4096² a 60p, un minuto: anunciaba 0,83 GB contra 1,5 GB reales; a 8192² se pasa 2,2× de más. Misma clase que el ×16 de HAP (R319), y desde R349 era el único camino a MP4. El `oninput` de `#exFfBr` llama ahora también a `exUpd()`: con sólo `exFfSync()` la cifra quedaba clavada justo cuando se la mira. `upd()` (live estimate + `#fmtChip`), `validateRes()` (probes `pickAvcCodec`/`pickHevcCodec`, greys `#exGo` with reason via `setDis`), `autoBr()`→`suggestBitrate`, `HAP_FMT`, `hapAutoChunks`, `exRangeSecs`. `$('#exGo').onclick` → `addJob`/`queueJob` → `pumpExportQ`.
...
- **Key symbols:** range resolution (`opt.range` 'inout'|'clips', `opt.rangeT` overrides for RIP); `eW/eH/qRes/dimStr/filePre` dims (wall vs flat vs dome-square); `seekExport(t)`→`vinstSeek`; `prepNests`; **[R330]** `collectDrawnVideoClips` pilota el medio EFECTIVO (`mediaEfId`) con el mismo reloj que usa `prepNests` al componer el nido — antes el compositor dibujaba el medio rotado y el decodificador servía los fotogramas del original; `renderExportFrame` (dome) / `composite` (png flat); `exportAudioMix`→**[R331]** `programarFundidos` (extraída para poder medirla: la salida no puede empezar antes de que acabe la entrada — con fi+fo > duración los eventos de ganancia salían desordenados y la entrada se cortaba de golpe; la previsualización ya lo clampaba y el export era el gemelo olvidado)/`audioBufferToWav`/`muxAudioAAC`/`audioPCM16`. **[R331]** el timecode de la chapa (`chapaTC`) se cuenta desde el ORIGEN DE LA SECUENCIA (`t`), no desde el principio del export: con marcas de entrada/salida empezaba en 00:00:00 aunque el tramo arrancara en el minuto tres. «Frames: N» sigue siendo el avance del render. Decodes source-clip audio tracks into `m._exAudio` (≤1.5 GB cap via `decodeAudioData`). `_rsSeq` switches to `opt.seqId` and restores.
...
- **[R188] Por qué el export va lento — MEDIDO sobre `untitled.isp` (domo 4096²@60, anillo de 6 sobre 2 archivos).** Reparto por fotograma: `seekExport` **80%** (498 ms), comprimir PNG 19% (118 ms), disco 1%, componer nests **0 ms**, composite final **0 ms**. La GPU no hace nada: el export está PARADO esperando a los `<video>`.
  - **Arreglado: `seekExport` deduplica.** Varios clips del MISMO medio en el MISMO instante —que es exactamente lo que hace una composición en anillo— reposicionaban N decodificadores para el mismo fotograma. Ahora se reposiciona uno por grupo y su fotograma se sube a la textura de los demás (~1 ms frente a ~80). **Medido en caliente: seek 498 → 96 ms, total 625 → 257 ms por fotograma (1,6 → 3,9 fps).**
    - **[R346b·c] La clave de agrupación afirma «mismo FOTOGRAMA», así que se toma como lo hace el decodificador.** Era la MILÉSIMA, cuyas fronteras no están alineadas con las del fotograma (dos clips a 2,29160 y 2,29170 caen en la misma cesta con la frontera del fotograma 55 entre medias, y al seguidor se le copian los píxeles del jefe sin volver a posicionar). Ahora `Math.floor(instanteDecod(local)*1e6)`, que es **literalmente el entero que recibe `keyForTime`**: «misma clave ⇒ mismo fotograma» pasa a ser demostrable. Con `Math.round` quedaba residuo (medido: 181 agrupaciones equivocadas en 20 M de parejas a 0,6 µs, 0 con `floor`, agrupando lo mismo). No es alcanzable con el compose de hoy —`regenComposeNest` crea las copias con el mismo `start`/`inP`/`speed`, así que `srcT` devuelve el mismo doble bit a bit— pero cuesta cero. **Ojo:** `instanteDecod` en la clave **no cambia ninguna partición** (sumar una constante entera de µs antes de redondear/truncar no puede); lo que arregló el fallo fue `1e3`→`1e6`, no la tolerancia.
  ...
  - **[R349] La secuencia PNG con fondo NEGRO moría antes del primer fotograma con `ReferenceError: q is not defined`.** [R343] escribió `if(planchar && q===glc){ … }` en el cuerpo del `else` para decidir si hacía falta un lienzo de planchado, y ahí `q` no existe: es el `const q = chapaLienzo(…)` de dentro de `renderFrame`/`lienzoPng`, dos funciones más abajo. Con `pngBg:'alpha'` (el valor de fábrica) la condición cortocircuita y no se nota; en cuanto se elige negro, el export entero se cae. La pregunta que R343 quería hacer —«¿`chapaLienzo` devuelve OTRO lienzo?»— sólo se responde llamándola, así que se responde donde se la llama: el lienzo de fondo se crea perezosamente (`bgListo()`) y se redimensiona contra `glc`, que hasta el primer `pintarFotograma` todavía no tiene su tamaño de export. Cubierto por `scratchpad/r349-revision.mjs` (bloque 3).
...
- **Sin ejecutar todavía:** HAP y fotograma suelto abren un diálogo NATIVO (`saveFile`) que un arnés por CDP no puede contestar; de esos caminos se verificó que las opciones llegan bien al motor, y el motor es el de siempre. **[R349] La secuencia PNG ya NO está en esa lista**: `opt.outDir` salta su diálogo, y `r349-revision.mjs` la exporta de punta a punta con fondo negro en cada pasada de redes — que es justo el camino que llevaba roto desde R343 sin que nadie lo notara, precisamente por estar anotado como «no ejecutable por CDP».
...
- **Location:** app.js · `makeProxy(m)` (~L3266), queue `enqProxy`/`pumpProxy` (~L3196-3200), consts `PMAX=960,PMBPS=12` (~L3186).
- **[R349] `_pxGen` la pone SÓLO `pumpProxy`, nunca un llamador.** Significa «se está generando AHORA» y es la guarda de `enqProxy` (`if(m._pxGen||proxyQ.includes(m))return`). La entrada «Generar proxy» del menú contextual la marcaba a mano ANTES de encolar —costumbre anterior a R326, de cuando nadie la miraba—, y R326 escribió la guarda encima de esa misma propiedad sin ver que ya había un llamador poniéndola: **desde entonces el medio jamás entraba en la cola y el menú no generaba nada**. R327 remató el enredo poniéndola también en `pumpProxy` (su sitio) y tampoco miró el llamador. El fallo es MUDO por diseño del progreso: `pumpProxy` traga la excepción y deja `proxyPct=-1`, que `updProxyUI` pinta como barra a 0 % con «…» — idéntico a un proxy recién empezado. Red con su caso rojo: `scratchpad/r349-cola-proxy.mjs` (120 s sin un solo %); `makeProxy` de punta a punta, incluso con material pesado real, en `scratchpad/r349-proxy.mjs`.
...
- **Location:** app.js · `makeClipDecoder(d,ex)` (~L8100). Per-clip glue `_vinstUrl`/`_useCD`/`vinstEnsure` (~L8355-8427), `vinstSeek`/`seekCDExport`/`vinstSeekVideo` (~L8495-8527), `driveCD` (~L8546). Demuxador: `demuxMP4` (~L8010). **[R346]** el instante que entra al decodificador: `TOL_DECOD`/`instanteDecod` (L1119, junto a `srcT`).
...
- **[R344] El centinela de `lastFedPts`, y por qué era una REGRESIÓN y no sólo un arreglo inerte.** R343 lo cambió de `-1` a `null` en los cinco sitios que lo LEEN, pero en `resetTo` dejó el `-1` del final y añadió un `null` al principio que ese `-1` pisaba. Como toda vida de un decodificador empieza pasando por `resetTo` (`dec` nace en `null`), el valor nuevo no llegaba a leerse: `lastFedPts` valía `-1` **desde cada reinicio hasta la primera muestra alimentada** — la ventana en la que se rellena el búfer de 4 MB. (Sólo esa ventana: la alimentación lo sobrescribe con el `pts` real.) Con `-1`, la guarda `lastFedPts!==null` de la rama de salto grande da verdadero y `targetUs > -1+2000000` se cumple para cualquier destino pasado el segundo 2 → cada vuelta de `step()` vuelve a reiniciar mientras el búfer se rellena. **MEDIDO** (`r344-atasco-7472.mjs`, GOP de 2 s): **10 recreaciones de `VideoDecoder` por salto a 2,1 s y a 7,5 s, contra 1 a 0,5 s y a 1,9 s**; el de 2,1 s decodifica muchos menos fotogramas que el de 1,9 s, así que la causa es el umbral y no la carga. Tras el arreglo: 0-1 por salto. **`lastFedPts=null` va lo PRIMERO de `resetTo`**, no lo último: detrás hay dos llamadas que pueden lanzar (`mkDec()` y `d.samples[feed].pts`), y una excepción tragada por el `catch` vacío de `pump()` dejaría `vaciado` de la generación anterior — con `vaciado` en `true`, `passed()` acepta CUALQUIER instante, que es el fotograma equivocado en silencio que R194/R261 impiden.
- **[R344b] Y la mitad que faltaba: la rama de salto grande reiniciaba DONDE YA ESTABA.** Comparaba sólo `targetUs > lastFedPts+2s`, sin mirar si el reinicio movía el arranque de decodificación. Como `resetTo` crea el decodificador y el bucle de alimentación se corta al llegar `decodeQueueSize` a 12 (~0,2 s a 60 fps), con el destino a más de 2 s de su fotograma clave `lastFedPts` no lo alcanza nunca y el ciclo es estable. **MEDIDO** (`r344-gop-mayor-2s.mjs`, GOP de 4 s): destinos a 2,5 · 3,9 y 3,9 s de su clave → **3794, 3802 y 3658 reinicios y los 10 s de `seekCDExport` agotados sin fotograma** → `_cdFail` → el medio entero al camino `<video>`. Con la guarda `keyBefore(decIdxForTime(targetUs))!==feedBase`: **645, 1062 y 1098 ms con 0-1 reinicios**. Es anterior a R343; lo tapaba el material — con GOP de 2 s la distancia destino→clave nunca llega al umbral. Fuerte candidato al pendiente del bucle pegado al final del archivo (`docs/NEXT.md`).
- **[R345b] El barrido de ids: `maxIdEnClip` + `maxIdEnMedio`, y NADA de enumerar a mano.** R345 unifico las dos barridas de MEDIOS y dejo tres huecos que la revision midio: el barrido de `autoItems` era INERTE (leia `state` cuando `resetProjDefaults()` acababa de vaciarlo, diez lineas antes de que el archivo instalara el suyo -> ahora lee de `obj`); `maxIdEnMedio` se dejaba `room.walls[].id`, `anim[].id`, `anim[].gid` y `mod[][].id`, todos nacidos de `uid()` y todos serializados -y `gid` se busca POR VALOR, asi que dos acordes con el mismo mueve los dos el mando de intensidad-; y los clips de primer nivel seguian con su propia copia del recorrido (`fxMx`) a cuarenta lineas. **Al anadir un contenedor que lleve `uid()`, va en `maxIdEnClip` o en `maxIdEnMedio`, y la red `r345-ids-legacy.mjs` lo cubre sin tocarla** porque cuenta ids con un recorrido generico del estado, no con una lista escrita a mano.
- **[R345b] `desalojado(k)`: el hecho, dicho una vez.** «El fotograma existio, se decodifico y se desalojo, y no vuelve solo». Lo leen los TRES sitios que lo necesitan: `passed()`, `frameAt` y el reinicio de `step()`. R344c lo dedujo dentro de `passed()` y confio la recuperacion al antiatasco de R256, que lo expresaba con otro predicado -y entre los dos quedaba una banda sin salida, porque con fotogramas B `lastFedPts` es la ultima muestra en orden de DECODIFICACION y no el pts mayor: 10 s, `_cdFail`, y el medio entero al camino `<video>` el resto de la pelicula-. Ademas el reinicio de fin de archivo es INMEDIATO (con un tope de un intento por destino): los 400 ms del detector de atasco existen para distinguir atascado de LENTO a mitad de archivo, y al final el decodificador esta demostrablemente terminado — ~4 minutos por capa en un bucle de 1 s sobre 10 minutos, peor en ping-pong.
- **[R345b] `frameAt` era el gemelo sin barrer.** R344c arreglo el fotograma equivocado en el camino de export y dejo intacta la puerta de la PREVISUALIZACION, que hacia lo mismo: con `wcDecode` encendido, un bucle sobre el final de una fuente ensenaba el vecino al reproducir y al arrastrar. Devolver `null` no congela nada — el llamador no sube textura ese fotograma y `step()` ya pidio el reinicio.
- **[R345] `uid()` es un contador plano y su re-siembra al abrir vive en `maxIdEnMedio(m)`, UNA vez.** Estaba escrita dos veces (el barrido general y el del camino legacy `obj.sequences`) y las copias divergieron: la legacy no miraba `nestMarkers` ni `nestGroups`, y como los medios de secuencia de ese camino se crean DESPUES del barrido general, esos ids no los contaba nadie. MEDIDO: un `.ise` cuyo ultimo objeto creado fuera un grupo o marcador dentro de una secuencia hacia que el siguiente `uid()` devolviera ese mismo id -> dos grupos con el mismo `groupId`, y R278 midio que un miembro con `slot` fuera de rango se ELIMINA en silencio. Red: `scratchpad/r345-ids-legacy.mjs`. **Al anadir un contenedor nuevo dentro de un medio, va en `maxIdEnMedio` y punto.**
- **[R345] Los ids de EFECTO no se mezclan entre clips, comprobado.** Las siete busquedas por id estan acotadas al clip y ningun camino concatena arrays de `fx` (`pasteAttrs` reemplaza el array entero), asi que compartir un id entre clips distintos es inocuo. Era una de las dos «no confirmadas» de la auditoria 2026-08-06.
- **[R344c] El atajo de fin de archivo de `passed()` no vale si el fotograma pedido EXISTIÓ y fue desalojado.** `feed>=N && vaciado` lo añadió R194 para que los instantes POSTERIORES al último fotograma no colgaran el export — ahí replegarse al anterior es correcto porque no hay otro. Pero un BUCLE que da la vuelta cerca del final pide un fotograma ANTERIOR a lo que queda en caché, y el atajo lo daba por bueno: `frameNear` devolvía el vecino. **No se veía porque el decodificador se rendía antes** (10 s → `_cdFail` → repliegue `<video>`, lento pero correcto); al cerrar eso en R344b el peligro pasó a real — **MEDIDO: 10 fotogramas equivocados de 72 parejas, el peor a 28,27 dB**. Ahora el atajo exige que no haya nada cacheado por delante del pedido; si lo hay, `passed()` devuelve `false` y el antibloqueo de R256 reinicia en su fotograma clave. Verificado: 0 equivocados, el peor a 102,32 dB (un píxel), y el caso pasa de 100-138 s (R261) a 20-21 s. Red: `scratchpad/r261-finarchivo.mjs`.
- **[R348·b] RETENER sin soltar estanca al decodificador — y eso, no un número, es lo que cierra la optimización
  del bucle.** Medido con la configuración EXACTA de la app (`{codec, description}` y nada más, como los tres
  decodificadores que existen): alimentando sin parar y **sin cerrar ningún fotograma**, la emisión se detiene a
  los **9** en un 960², **16** en un 1440p y **6** en otro 1440p de material real
  (`scratchpad/r348-fondo-salida.mjs`; el control, la misma alimentación cerrándolos, llega a 40+ en los tres).
  Ensanchar `BEHIND` para que un ciclo de bucle quepa es entrar en ese régimen, y es literalmente lo que R257
  implementó y midió como la rendición de 980 ms/fotograma. La propuesta está cerrada por el **mecanismo**, no
  por la cifra. **Y sí hay red que lo vigila, contra lo que R348 dio por hecho:** `scratchpad/r261-finarchivo.mjs`
  exporta un bucle real y desde R344c lee `m._cdFail` metiéndolo en su código de salida (`r.rendido`), así que
  ensanchar `BEHIND` la pone ROJA — la rendición es precisamente cómo se manifiesta el estancamiento.
  **[R348b] Y la cifra NO es un techo de lo que la app puede tener abierto, aunque R348 lo escribió así.**
  Medido el pico real de `cache.size` recorriendo un tramo como lo recorre el export
  (`scratchpad/r348-pico-anillo.mjs`): **7 · 5 · 15** en los mismos tres archivos — o sea que con `o1.mp4` el
  anillo llega a **15 retenidos donde la pasada de retención se estancaba en 6**, y el export funciona igual. Son
  dos regímenes distintos: la pasada de retención no suelta nunca, y el anillo **recicla** (al avanzar el
  objetivo, `evict` cierra los de detrás y los búferes vuelven al fondo). Poner los dos números uno al lado del
  otro —y de ahí deducir que el anillo está «pegado al límite»— era una comparación inválida; queda retirada.
  Lo que sí se sostiene: (a) el mecanismo de arriba; (b) el límite del régimen de retención **no escala con el
  tamaño de fotograma** (16 y 6 en dos archivos del mismo tamaño), así que ninguna guarda por megapíxeles puede
  predecirlo — es del driver y del perfil. Para un clip muy loopeado la respuesta es hornearlo (*Render in
  place*), no cachearlo.
- **[R346] Un export a la cadencia de la fuente repetía y saltaba un fotograma de cada tres, por LOS DOS caminos — `TOL_DECOD` (2 µs).** El bucle del export pide `t = t0 + i/fps` y `srcT` lo pasa a tiempo local, así que en un máster 1:1 el instante cae en el comienzo EXACTO de cada fotograma. Ahí `keyForTime` compara contra `Math.floor(t_µs)` y Chromium trunca igual por dentro, mientras el pts de verdad tiene microsegundos fraccionarios (55/24 s = 2291666,667 µs): con `i/fps` la cuenta acierta cuando el cociente es exacto en binario (21/24 = 0,875) y falla cuando no. **MEDIDO** sobre un tramo seguido de 40 fotogramas: **13 repetidos y 13 saltados** a 24 fps (`90,90,91,93,93,94,96,96,97,99,99,100…`) y 1 y 2 a 60 fps. **Por qué sobrevivió tanto:** los dos caminos se equivocan EN LO MISMO —0 diferencias entre ellos en 54 comparaciones—, así que toda verificación que enfrente el ClipDecoder con `<video>` sale «idéntico bit a bit», que es literalmente lo que concluyó R189; sólo lo caza un criterio SIN ORÁCULO (en un export 1:1 los fotogramas van de uno en uno). La tolerancia se suma **una sola vez por posicionamiento**, y en `vinstSeek` va **ANTES de bifurcar**, para que los dos caminos reciban el mismo número y el máster no cambie de fotograma si un medio cae al repliegue. **[R346b] `vinstSeek` NO es el único sitio** —decirlo así en el comentario fue lo que dejó cuatro sitios sin barrer—: la lista completa vive en la nota de `instanteDecod` (L1117) y la regla es «todo instante que se le pide a un decodificador pasa por ahí, una vez». **Por qué 2 µs:** es la suma de las dos cuantizaciones a microsegundo entero del camino (la nuestra y la de Chromium) más el ruido de `i/fps`; medido, con 1 µs todavía se cuela un repetido a 60 fps y los caminos discrepan en dos instantes. **Por qué NO el centro del fotograma**, que salió igual de limpio en el banco: exige rejilla uniforme a `fps` y `detectFps` canonizaba 59,94 a 60 → sobre material NTSC el centro caía en el fotograma equivocado a partir de los ~8 s; el daño de la tolerancia está ACOTADO a 2 µs pase lo que pase con la cadencia. **[R347] Honestidad sobre ese argumento:** la mitad del NTSC ya no aplica —`detectFps` devuelve ahora la cadencia real—, así que lo que sostiene la decisión es lo otro, que sigue entero: el daño acotado, y que el centro necesita una rejilla uniforme que un archivo VFR no tiene. Medido además que la tolerancia aguanta las tres cadencias NTSC (0 desplazados en los dos caminos, `r347-ntsc.mjs`). Gemelos barridos: `snapKf` (el pts exacto del fotograma clave, mismo caso), `driveCD` (reproducción) y las dos secuencias de imágenes (`drawClip` y el monitor de origen), que elegían con `Math.floor(srcT·fps)` y `55/24·24` da 54,99999999999999. **[R346b] Y los cuatro sitios que faltaban, que encontró la revisión:** `play()` y el servo de deriva de `ploop` escribían el instante CRUDO en el mismo `<video>` —o sea que dar al play arrancaba un fotograma por detrás de lo que se veía en pausa, y cada vuelta del bucle enseñaba el anterior al punto de entrada—; las **tres** escrituras del monitor de origen (sólo se había barrido su rama de secuencia de imágenes; detalle en la fila «Monitor de origen»); y el generador de proxys, donde el fotograma equivocado se HORNEA en el archivo — **[R346c] ahí la tolerancia vive dentro de `seekRaw`**, que es la función que de verdad posiciona, no en su llamador. La clave de agrupación de `seekExport` NO es de esta familia (no escribe a ningún decodificador): su arreglo está en la ficha del pipeline de export. Red: `scratchpad/r346-verif.mjs` (se comprueba a sí misma pidiendo `t − TOL_DECOD`, que reconstruye el estado anterior sin tocar app.js: 13 y 13 otra vez). Sondas del diagnóstico: `r346-quien-elige.mjs`, `r346-mapa-fotogramas.mjs`, `r346-video-carrera.mjs`.
- **[R346] Y la hipótesis que llevaba un año escrita en `docs/NEXT.md` es FALSA:** el repliegue `<video>` no tiene una carrera con la presentación. Medido con `requestVideoFrameCallback` sobre el elemento en pausa: la presentación llega **antes** de `seeked` en 8/8 posicionamientos y el píxel no cambia después en ninguno; cuatro elementos posicionándose a la vez (lo que hace `seekExport`) dan los 24 iguales, y tres pasadas en distinto orden repiten los 16. Lo que sí variaba de instante a instante era la elección de fotograma en la frontera, que es lo de arriba.
- **[R344] El `have` de la rama de reinicio hacia atrás (`step()`) es inalcanzable con GOP CERRADO.** Exige un fotograma cacheado en `[destino−2f, destino]` con el destino ANTERIOR a `feedBasePts`; pero `resetTo` vacía la caché, así que todo lo cacheado es ≥ `feedBasePts` > destino. Medido desde seis puntos de asentamiento: caché siempre CONTIGUA (0 huecos), **0 candidatos**. Por eso el atasco 7472 del inventario no se reproduce: su premisa —que el vecino impide el reinicio— no se cumple. **La guarda se queda**: con GOP ABIERTO sí puede haber muestras por debajo del fotograma clave, y cuesta ≤24 vueltas. No borrarla por "inalcanzable".
- **Status:** ✅ en export (verificado contra `<video>` por PSNR, H.264 y HEVC 10 bits; **[R344] y al FOTOGRAMA**, con un recorte real de 14 fotogramas de edit list) · 🚧 en previsualización (pendiente de mover a un worker)
...
- **[R345/R345b] La re-siembra de `_id` al abrir vive en `maxIdEnClip`/`maxIdEnMedio`** (~L11930-11955), y la usan los DOS sitios que la necesitan: el barrido general y el del camino legacy (`obj.sequences`). Tenerla copiada fue lo que produjo la divergencia de R345, y la barrida de `autoItems` que hay al lado era INERTE (leía `state` en vez de `obj`). **Al añadir un contenedor que lleve `uid()`, va ahí.** Detalle en la fila del ClipDecoder y en `PLAN.md › ROUND 345/345b`; red: `scratchpad/r345-ids-legacy.mjs`.
