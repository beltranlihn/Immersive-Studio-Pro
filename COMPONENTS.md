...
- **Key symbols:** classes `.lanehdr .sel .collapsed .aud`; buttons `[data-m=collapse|mute|solo]`, `.laneres [data-m=resize]`. `laneH(li)`. Constantes **del diseño (R152)**: `LANE_DEF_H=57, LANE_MIN_H=26, LANE_MAX_H=120, LANE_COLLAPSED_H=24`, `AUDIO_LANE_H=44` (default del audio, NO una altura fija), `RULER_H=24`, `TRACK_COLORS`. **[R163]** `AUTO_LANE_MIN_H=52` = suelo en modo automatización para pistas de vídeo (los desplegables de identidad necesitan margen); `laneFloorH(l)` es la fuente única y la usan `laneH`, `wheelResizeLanes` y el arrastre de la barra vertical. Por debajo del suelo la pista NO se queda a medias: `collapsed=true`. `lanesTopDown()` display order.
