...
| `roomPlan` | Lazo de muros → planta; [R211] envuelve en sentido natural (desde dentro mirando Front, Right a la DERECHA = lado x−); **[R232]** de las dos raíces de θ se coge la que NO se cruza (+ `segCruza`/`planCruzada`) | app.js · `roomPlan` (~L7143) | ✅ | — |
...
- **[R232b] SIN RESOLVER (decisión de diseño, ver `docs/NEXT.md`):** reordenar muros re-coloca `x0/x1` pero **no mueve el contenido ya colocado** — un clip con «Mask to wall» se recorta al hueco NUEVO de su muro (`roomWallScissorRects`) mientras sus píxeles siguen en el viejo, y sale en blanco. Pre-existente: el selector de orientación de antes intercambiaba los muros de hueco igual.
