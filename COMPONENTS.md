...
| `makeProxy` | Proxy all-intra GOP=1 + m.frames. **[R241] su tasa sale de `m.fps`** → si `detectFps` falla, el proxy nace a media tasa (medido: 455 de 911 fotogramas) y el montaje enseña un fotograma de cada dos. **[R346b·c]** su camino de seek pide la rejilla `i/fps`, o sea la frontera exacta de cada fotograma, y aquí el fotograma equivocado se HORNEA en el archivo: la tolerancia la aplica ahora `seekRaw`. Es el TERCER camino (el rápido de WebCodecs va por índice y es inmune, el de rVFC se pilota por `mediaTime`), así que sólo muerde cuando el demuxador o el códec fallan. Pendiente ahí mismo, y mayor: `detectFps` canoniza 59,94 a 60, con lo que sobre material NTSC esta rejilla deriva | app.js · `makeProxy` (~L2524) · `seekRaw` | ✅ | [C3] · R346c |
