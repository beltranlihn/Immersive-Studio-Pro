...
| `makeProxy` | Proxy all-intra GOP=1 + m.frames. **[R241] su tasa sale de `m.fps`** → si `detectFps` falla, el proxy nace a media tasa (medido: 455 de 911 fotogramas) y el montaje enseña un fotograma de cada dos. **[R347] y si `m.fps` está CANONIZADA, la rejilla deriva**: `detectFps` redondeaba 59,94 a 60, el error relativo es 1/1001 y la deriva alcanza medio fotograma en `1001/(2·fps)` s (8,3 s a 59,94), creciendo un fotograma más cada `1001/fps`. Medido al 60 % de un archivo NTSC de 30 s: **40 de 40 fotogramas desplazados**. En un clip de 10 min el final del proxy va ~36 fotogramas por detrás del original. Arreglado en `detectFps`; red `scratchpad/r347-ntsc.mjs`. **[R346b·c]** su camino de seek pide la rejilla `i/fps`, o sea la frontera exacta de cada fotograma, y aquí el fotograma equivocado se HORNEA en el archivo: la tolerancia la aplica ahora `seekRaw`. Es el TERCER camino (el rápido de WebCodecs va por índice y es inmune, el de rVFC se pilota por `mediaTime`), así que sólo muerde cuando el demuxador o el códec fallan | app.js · `makeProxy` (~L2524) · `seekRaw` | ✅ | [C3] · R346c · R347 |
| `detectFps` | Tasa real del vídeo por `requestVideoFrameCallback`. **[R241]** al vencer el plazo YA NO tira las muestras (tres intervalos bastan para la mediana) y el plazo escala con los megapíxeles hasta 8 s: con 2,5 s fijos, un HEVC de 7196×912 a 410 Mbps no llegaba a diez fotogramas y se quedaba en el 30 por defecto — **0 de 9 aciertos en el material real de Beltrán, ahora 9 de 9**. **[R347] NTSC:** hacía `canon(Math.round(1/md))`, y ese `Math.round` —anterior a la tolerancia de `canon`— borraba 59,94 → 60 y 23,976 → 24. Ahora la mediana entra en `canon` SIN redondear y el juego de tasas lleva las tres NTSC, eligiendo la más CERCANA. Se puede distinguir porque `mediaTime` es el pts exacto: la mediana vale 1001/30000 clavado y 59,94 dista 0,06 de 60. **Medido en los dos sentidos** (`r347-ntsc.mjs`): NTSC → 23,976/29,97/59,94, y material de 24 y 60 EXACTOS → 24 y 60, que es el riesgo que el propio arreglo introduce. Por qué importa: `m.fps` manda en la rejilla de seek del proxy, y con la canonizada el proxy deriva un fotograma cada `1001/fps` s | app.js · `detectFps` (~L2467) | ✅ | R241 · R347 |
...
Source: `app.js` (~15952 lines). Line numbers verified against the working tree on 2026-08-09 (R345b).
