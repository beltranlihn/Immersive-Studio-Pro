...
- **[R348] La capacidad del FONDO DE SALIDA del decodificador, medida: 6 a 16 VideoFrames.** Es el límite duro de
  todo este subsistema y explica tres decisiones que parecen arbitrarias. Medido alimentando un `VideoDecoder`
  crudo y contando los fotogramas que llegan **sin cerrarlos** (`scratchpad/r348-fondo-salida.mjs`; el control,
  la misma alimentación cerrándolos, llega a 40+ en los tres archivos): **9** retenidos a 960², **16** en un
  1440p y **6** en otro 1440p de material real. Consecuencias: (a) el anillo de export vive en 2 detrás + 6
  delante ≈ **8 en caché, pegado al límite** — de ahí que `MINC` sea imprescindible y que no haya holgura para
  ensancharlo; (b) **retener un ciclo de bucle es imposible**, no una cuestión de afinar constantes: 12
  fotogramas no caben en dos de los tres casos y sobre material real caben 6 (esto es lo que R257 midió como la
  rendición de 980 ms/fotograma, y lo que cierra definitivamente la optimización que la cola proponía); (c) el
  límite **no escala con el tamaño de fotograma** —16 y 6 en dos archivos del mismo tamaño—, así que ninguna
  guarda por megapíxeles puede predecirlo: es del driver y del perfil. Para un clip muy loopeado la respuesta es
  hornearlo (*Render in place*), no cachearlo.
