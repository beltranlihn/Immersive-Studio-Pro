...
- **Deuda anotada:** los visores siguen siendo los painters ESQUEMÁTICOS, no los viewports reales del editor (pedido de Beltrán: "traerlos a este diagrama nomás"). El 3D del editor es WebGL sobre `#gl`; el camino es componer a `glc` y blitear con `drawImage` al canvas del panel (el mismo truco del visor de avance de RIP, `ripProgress.frame`). El domo 3D queda además ligeramente descentrado (`cy=H/2+8*U`). En la tira cosida, el total que dibuja `drawRoomStrip` se solapa con la etiqueta del último muro a este alto (comportamiento del painter compartido con el diálogo de sala; no se tocó para no cambiar el diálogo).
