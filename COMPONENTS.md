...
| ClipDecoder | Anillo WebCodecs decode-ahead | app.js · `makeClipDecoder` (~L4026) | ✅ export · 🚧 preview | R189 · [C2] |
...
| `newProject` | Reset + crear proyecto domo/flat. **[R227] devuelve `true`/`false`** (false = `confirmDiscard` canceló) y ya NO lanza el recorrido guiado | app.js · `newProject` (~L8226) | ✅ | — |
| `newProjectViaLanding()` [R227] | File → «New project…» / Ctrl+N: pregunta de tres salidas (`appConfirm3`: guardar/descartar/cancelar) si hay cambios y lleva a la **pantalla de inicio**, dejando el proyecto abierto intacto detrás («Back to project» lo devuelve) | app.js · junto a `confirmDiscard` (~L8218) | ✅ | R227 |
| `startDemoProject(fmt)` [R227] | Construye en memoria un proyecto DEMO del formato pedido (formas + texto, 4 pistas, composición, Motion, Efecto, automatizaciones) y lanza el recorrido guiado encima. Botón «Demos» de la pantalla de inicio | app.js · `startDemoProject`/`_demoBuild*`/`_demoFinish` (~L3577) | ✅ | R227 |
...
| Diálogos estilados | appPrompt/appAlert/appConfirm + **[R227] `appConfirm3`** (guardar/descartar/cancelar → `'save'|'discard'|'cancel'`, z-360) | app.js · (~L2939-3005) | ✅ | — |
...
- **Location:** app.js · `trackKeyFor`, `focusAutoParam`, `showAutomationParam`, `clipArmedTrackKeys`; hooks in `buildRows` (field pointerdown / box dblclick / box wheel / diamond click / row contextmenu), `wireFxCards` (`.fxrow` pointerdown / dblclick / kf click / contextmenu) and `buildAnimList` (Mix slider + `.awetkf`)
...
- **Key symbols:**
  ...
  - `drawMaskEditOverlay()` — polígonos + puntos del activo sobre `gx`, llamado al final de las DOS ramas de `drawGrid2D`; incluye la pill de modo (patrón R219/R220, baja a y=46 si la de «Preparando medios…» ocupa la fila).
...
- **Location:** app.js · `roomSetupDialog(cb)` (~L5127); `roomPlan(walls)` (~L5033); `drawRoomIso(cv,walls,floorOn,activeRole,pal,solo)` (~L5051); `drawRoomStrip` [F5] (~L5113); presets `getRoomPresets`/`saveRoomPresets` (L5110/5111)
...
- **Key symbols:** `roomPlan` → footprint segments `{role,a,b,h}` in METERS + `poly` + `closed` + `imposible`; los ángulos salen de las medidas, ninguno se fija a mano. `#rsN` wall count, `#rsWalls` rows, `#rsFloorRow`, `#rsStrip` [F5] canvas.
...
- **Location:** app.js · `newRoomProject(cfg)` (~L8290). **[R227]** devuelve `true`/`false` (false = `confirmDiscard` canceló) y ya no lanza el recorrido guiado.
...
- **Location:** app.js · demos: `DEMO_DUR`/`_demoRefs` · `_demoPlace`/`_demoAddShape`/`_demoAddText`/`_demoKf`/`_demoMotion`/`_demoFx`/`_demoCompose`/`_demoLaneName`/`_demoRoomPos` · `_demoBuildDome`/`_demoBuildFlat`/`_demoBuildRoom` · `DEMO_LABEL`/`startDemoProject(fmt)`/`_demoFinish`/`buildDemoProject` (bloque tras `lchCreate`, ~L3577). Recorrido: `tourTrasCrear(fmt,demo)` / `startTour(fmt,demo)` / `tourSteps(fmt,demo)` / `_demoSelect(clipId,tab)`. Disparos: `startDemoProject` (único automático) e ítem de menú en `openAppMenu('window')`.
...
- **Key symbols:** paso = `{sel,title,body,act?,reveal?}`. **`act`** deja el editor en el estado del que habla el paso (`_demoSelect` selecciona el clip y cambia de pestaña; el paso de automatización enciende `state.inlineCurves` y fija `lane._autoP`) — se ejecuta en CADA `draw`, así que es IDEMPOTENTE y no llama a `markDirty` (enseñar no es editar). **`sel` admite función** (los ids de clip del demo son de tiempo de ejecución) y **`reveal`** hace `scrollIntoView` antes de medir. `draw()` coloca el agujero sobre `getBoundingClientRect(sel)` (centrado a pantalla completa si `sel:null`); teclas Esc/←/→/Enter; `resize` redibuja.
...
- **Invariants / gotchas:** built on `openMenu`; the active `.menubtn` highlight is cleared by `closeMenu` (R135). **[R227] `_descartarYaDicho`** = bandera de UN SOLO USO que consume la siguiente `confirmDiscard()`: sin ella la misma pregunta salía dos veces (la de `appConfirm3` y la del `newProject` que lanza el launcher). NO se toca `state.dirty`, para que «Back to project» devuelva el proyecto con sus cambios y su asterisco; la bandera la limpia el propio botón de volver. `appConfirm3` va a **z-360** (por encima del launcher, z-300).
- **Status:** ✅
...
- **State/data:** `_viewerWin`, `_viewerCtx` (2D ctx del lienzo de la emergente), `_vMode` (`auto|2d|3d`) + `_vEditorMode`, `_vCam` (cámara propia: `yaw/pitch/dist/fov/back`, sirve para domo y sala), `_vThree` (`orbit|spec`), `_vGrid`, `_vOverlay`, `_vDirty`, `_vRaf`, `_vPaint`, `_vBusy`, `_vBarSig`, `_viewerBar`. Además `_reuseComp`/`_lastSrcTex` en `render()`.
...
- **Key symbols / diseño nuevo:**
  ...
  - Barra `#vwbar` dentro de la emergente, **CSS auto-contenido**: segmento `2D|3D` (override manual que se suelta en cuanto el editor cambia de modo), `Grid`, el overlay contextual (`Horizon` domo / `Seam` sala / `Center` 2D) y en 3D el segmento `Orbit|Viewer`. Se reconstruye por firma (`modo efectivo|seqMode|toggles|idioma`). Arrastre = orbitar, rueda = `dist` (Orbit) o `back` (Viewer).
- **Invariants / gotchas:** `viewerHas3D()` = domo o sala; una secuencia 2D plana no tiene 3D, así que la ventana se queda en 2D y su segmento 2D/3D desaparece (igual que en el editor). El botón de overlay se oculta en **sala + 3D** (la costura es una guía de la tira 2D). La ventana es una **salida limpia**: `zoom=0.94`, `pan=[0,0]`, sin contornos, sin scopes, sin alfa, sin tiradores ni overlay de máscara (`_vPaint` los apaga). `_vBusy` cubre también el render de cierre: sin eso, marcarse sucio a sí mismo sería un bucle de repintado eterno. Coste medido con la ventana abierta y 10 s de reproducción: **~42 fps** de editor (el resto es Chromium componiendo dos ventanas en la GPU del dev; el `.exe` fuerza la RTX). Camino viejo archivado con el desglose de la medición en `_backup/deprecated/20260730-viewer-window-readback.js`.
- **Status:** ✅
- **Roadmap:** —
