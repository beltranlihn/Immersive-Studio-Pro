...
- **Key symbols:**
  ...
  - **[R231] `maskSegHit(px,py)`** — arista bajo el cursor: `{si, x,y (punto proyectado), ax,ay,bx,by}`, tolerancia 8 px (un pelo por debajo de los 9 px del vértice, para que el vértice gane el empate) y los extremos (`u<6%` / `u>94%`) cedidos al vértice. `_maskHover`/`_maskHoverSig` guardan el hover; la firma se redondea a píxel entero para no repintar en cada sub-píxel. `maskHoverClear()` lo apaga (`endMaskEdit`, `pointerleave` de `gridc`, y al insertar/agarrar).
