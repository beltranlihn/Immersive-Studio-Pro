...
| Gesto de mover | Move/copy de clip con ghost. Partner A/V enlazado: sólo horizontal (`primaryIds` vs `items`). **[R231]** la pista destino se valida por la PISTA del clip (`isAudioClip`), no por el medio. **[R349] umbral de arrastre `TL_UMBRAL`=6 px**: hasta que el puntero no se aleja esa distancia, `onTLMove` no hace nada. No había ninguno — seleccionar y mover eran el mismo gesto desde el primer píxel, así que el temblor de la mano al hacer clic ya desplazaba el clip, y con la línea apretada un píxel son varios fotogramas. Mismo orden que los umbrales que el editor de curvas (3-4 px) y el arrastre de carpetas (5 px) ya llevaban a mano | app.js · `onTLMove`/`onTLUp` · `TL_UMBRAL` | ✅ | R349 |
...
| **Play fuera del tramo** [R279 · R286b · R349] | Dos decisiones que se toman **al arrancar**, no cada fotograma: `_enBucle` (¿envuelve? sólo si el cabezal cae DENTRO del tramo) y **[R349] `_paraEnOut`** (¿hay que parar en `workOut`? sólo si el cabezal arranca DELANTE de él). R286b repuso la parada que R279 se llevó de más, pero mirando sólo `playhead>=workOut` — cierto desde el primer fotograma cuando se da al play DETRÁS del tramo, que es donde suele quedar el cabezal tras repasar el final: volvía a `workOut` y pausaba en el acto. Reproducir más allá del bucle y más allá del último clip (que es lo que R279 vino a permitir) vuelve a funcionar, y parar al SALIR del tramo yendo hacia adelante se conserva | app.js · `play()` / `ploop()` · `_enBucle` / `_paraEnOut` | ✅ | R349 |
