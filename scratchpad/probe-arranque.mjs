...
const linea = [];
let vioLoadingOv = false, revelado0 = null, proyecto0 = null;
for (let i = 0; i < 140; i++) {
  ...
  if (s && !s.ROTO) {
    ...
    console.log(String(Date.now()-t0).padStart(6)+"ms  preboot="+(s.preboot?1:0)+" esperando="+s.esperando+" revelado="+s.revelado+" pct="+s.pct+" loadingOv="+(s.loadingOv?1:0)+" clips="+s.clips);
