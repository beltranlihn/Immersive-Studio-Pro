...
const r=await evl(`(()=>{ const conKf=state.clips.filter(c=>c.kf&&Object.keys(c.kf).length).map(c=>c.id);
  const cd=[...document.querySelectorAll('.clip')].find(x=>conKf.includes(+x.dataset.clip)&&x.querySelector('canvas'));
...
fs.writeFileSync('scratchpad/shots/17b-auto-con-curva.png', Buffer.from(s.data,'base64'));
