...
const DIR=path.join(process.cwd(),'scratchpad','r255-diag');
fs.rmSync(DIR,{recursive:true,force:true}); fs.mkdirSync(DIR,{recursive:true});
console.log(await ev(`(async()=>{ state.clips=[];
  const V=state.media.find(m=>m.kind==='video'); const vl=state.lanes.findIndex(l=>l.kind==='video');
  const c=makeClip(V,vl,0); c.dur=1; c.inP=4; c.props.el=90; c.props.size=90; state.clips.push(c);
  state.selId=c.id; state.selIds=[c.id]; toggleLoop(c); setLoopRange(c,0.4); c.dur=1;
  renderTimeline(); render();
  const ui=ripProgress('diag','30 fotogramas',1);
  await runExport({codec:'png', res:512, fps:30, range:'clips', rangeT:[0,1], outW:512, outH:512,
  ...
  return { cdFail:!!V._cdFail, instancias:_vinst.size, stats:st }; })()`));
ws.close();
