...
console.log(JSON.stringify(await evl(`(()=>{
  const s=document.getElementById('statRight')||[...document.querySelectorAll('#status *')].find(e=>/clips/.test(e.textContent||''));
  const antes=s?s.textContent.replace(/\s+/g,' ').trim():'no encontrado';
  try{ updStatus(); }catch(e){}
  const despues=s?s.textContent.replace(/\s+/g,' ').trim():'no encontrado';
  return { clipsReales:state.clips.length, mediosReales:state.media.length, barraAntes:antes, barraTrasUpdStatus:despues };
})()`),null,2));
ws.close();
